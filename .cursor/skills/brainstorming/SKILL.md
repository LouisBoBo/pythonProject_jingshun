...
6. **编写设计文档** — 保存到 monorepo 根目录下的 `.superpowers/specs/YYYY-MM-DD-<topic>-design.md` 并 commit
...
- 将验证通过的设计（规格说明）写入 monorepo 根目录下的 `.superpowers/specs/YYYY-MM-DD-<topic>-design.md`
  - **路径始终相对于 monorepo 根目录**，不受当前操作的子项目位置影响
  - （用户对规格位置的偏好优先于此默认值）
