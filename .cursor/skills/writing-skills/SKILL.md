...
**个人技能存放在智能体特定的目录中（Claude Code 用 `~/.claude/skills`，Codex 用 `~/.agents/skills/`，CodeBuddy Code 用 `~/.codebuddy/skills/`）**
...
**重要：使用 TodoWrite 为下面的每个清单项创建待办。**（CodeBuddy Code 用 `TaskCreate` / `TaskUpdate`）
